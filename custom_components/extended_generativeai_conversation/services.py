...
QUERY_IMAGE_SCHEMA = vol.Schema(
    {
        ...
        vol.Required("model", default="gemini-2.0-flash"): cv.string,
...
async def async_setup_services(hass: HomeAssistant, config: ConfigType) -> None:
    ...
    async def query_image(call: ServiceCall) -> ServiceResponse:
        ...
        try:
            ...
            # Get configuration parameters if available
            config_entry = hass.data[DOMAIN][call.data["config_entry"]]

            # Configure the Generative AI with all necessary parameters
            config = {"api_key": api_key}

            # Add base_url and api_version if they exist in the config entry
            if "base_url" in config_entry:
                config["client_options"] = {"api_endpoint": config_entry["base_url"]}
            if "api_version" in config_entry:
                config["api_version"] = config_entry["api_version"]
